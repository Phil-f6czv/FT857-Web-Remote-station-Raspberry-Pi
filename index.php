...
function displayfreq(Str_frequency) {
    ...
    var MHz =  "";
    var kHz = Str_frequency.slice(length-6,length-3);
    var dHz = Str_frequency.slice(length-3,length-1);
    if (length >= 7) {
    MHz = Str_frequency.slice(0,length-6);
    Formatted_freq = MHz + "." + kHz + "." + dHz;
    }
    else {
    Formatted_freq = kHz + "." + dHz;}
    document.getElementById("freq").innerHTML = Formatted_freq;
}

function displaymode(Str_mode) {
...
function displayVFO(Str_VFO) {
   var VFO1 = "";
   VFO1 = Str_VFO;
   currentVFO = VFO1.charAt(3); 
   document.getElementById("vfo").innerHTML = currentVFO;
   }

function displayRxTx(Str_RxTx) {
...
if (Str_RxTx != 0) {document.getElementById("redimg").style.display = "block";
                                   RxTx = 1;
...
function displaySplit(Str_Split) {
      var split;
      split = Str_Split.charAt(0);
      if (split != "0") {
...
document.getElementById("smeter").innerHTML = Smeter_val;
...
function toggleVFO () {
  ...
  xhttp.open("GET", "togglevfo.php", true);
  xhttp.send();
}
...
function togglesplit () {
  var xhttp = new XMLHttpRequest();
  if (SPLIT == false) {
  xhttp.open("GET", "rigctl.php?q=S 1 VFO", true);
  SPLIT = true;}
  else
  {xhttp.open("GET", "rigctl.php?q=S 0 VFO", true);
  SPLIT = false;}
  xhttp.send();
}
...
function toggleclar () {
  var xhttp = new XMLHttpRequest();
  if (CLAR == " ") {
  xhttp.open("GET", "rigctl.php?q=J 1", true);
  document.getElementById("clar").innerHTML = "-"; 
  CLAR = "-";}
  else
   {xhttp.open("GET", "rigctl.php?q=J 0", true);
   CLAR  = " ";
   document.getElementById("clar").innerHTML = " ";}
  xhttp.send();
   }
...
/* This implementation was improved thanks to OK1ZIA : only one request is sent to get all the parameters .
 */

setInterval(function ( ) {

  /* calculation of the frequency variation done by the VFO dial.
  If not 0 sends a request of frequency update to the server */
...
/* processing of the response : the actual value sent by the server is processed if the response has a valid status.
The parameters sent by the server (by the HAMLIB library) are stored in an array, each parameter separated by a \n 
The first thing is to get each parameter by splitting the array */

  xhttp1.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
    var resp = xhttp1.responseText.split("\n");
    displaymode(resp[0]);
    displayVFO(resp[2]);
    displaySmeter(resp[3]);
    displayRxTx(resp[4]);
    displaySplit(resp[5]);
    curfreq = Number(resp[7])/10;
    displayfreq(resp[7]);
    
  }}
// request of mode, VFO, Smeter, Rx/Tx status, Split status, frequency
xhttp1.open("GET", "rigctl.php?q=m+v+l STRENGTH+t+s VFO+f", true);
  xhttp1.send();
 

  /* The following paramaters (DSP, CW) are not available for a FT857 using HAMLIB
   The source code is left in comments for a future use
